...
$$L(\lambda) = \sum_1^n (log(\lambda^{k_i}) + log(e^{-\lambda}) - log(k_i!)) $$
$$ = \sum_1^n (k_i *log (\lambda) -\lambda - log(k_i!) )$$ Plugging in a few values, we see a general curve appear below!
